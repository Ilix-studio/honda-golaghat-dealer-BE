...
exports.validateMultipleImageFiles = exports.validateImageFile = exports.optimizeImageUrl = exports.generateResponsiveImageUrls = exports.deleteMultipleFromCloudinary = exports.deleteFromCloudinary = exports.uploadToCloudinary = exports.uploadMultipleBikeImages = exports.uploadBikeImageToCloudinary = void 0;
const cloudinary_1 = require("cloudinary");
// Enhanced Cloudinary upload for bike images with multiple transformations
const uploadBikeImageToCloudinary = (buffer, originalName, bikeModelName, index = 0) => {
    return new Promise((resolve, reject) => {
        const publicId = `bike_${Date.now()}_${bikeModelName.replace(/\s+/g, "_")}_${index}`;
...
exports.uploadBikeImageToCloudinary = uploadBikeImageToCloudinary;
// Upload multiple bike images with proper naming and optimization
const uploadMultipleBikeImages = async (files, bikeModelName) => {
    const uploadPromises = files.map((file, index) => (0, exports.uploadBikeImageToCloudinary)(file.buffer, file.originalname, bikeModelName, index).then((result) => ({
        ...result,
        isPrimary: index === 0, // First image is primary
    })));
    return Promise.all(uploadPromises);
};
exports.uploadMultipleBikeImages = uploadMultipleBikeImages;
...
const deleteFromCloudinary = async (imageUrl) => {
    try {
        ...
        if (result.result !== "ok" && result.result !== "not found") {
            console.warn(`Cloudinary deletion warning: ${result.result} for ${publicId}`);
        }
    }
    catch (error) {
...
const generateResponsiveImageUrls = (cloudinaryPublicId) => {
...
const optimizeImageUrl = (originalUrl, width, height) => {
...
exports.validateMultipleImageFiles = validateMultipleImageFiles;

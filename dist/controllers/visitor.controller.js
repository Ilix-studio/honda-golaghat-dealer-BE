...
exports.resetVisitorCount = exports.getVisitorStats = exports.getVisitorCount = exports.incrementVisitorCount = void 0;
 ...
 * @access  Public
 */
exports.incrementVisitorCount = (0, express_async_handler_1.default)(async (req, res) => {
    try {
        // Get or create visitor counter document
        let visitorData = await visitor_1.default.findOne();
        if (!visitorData) {
            // Create new visitor counter if it doesn't exist
            visitorData = await visitor_1.default.create({
                totalVisitors: 1,
                lastVisit: new Date(),
                dailyVisits: [
                    {
            ...
            const todayEntry = visitorData.dailyVisits.find((visit) => visit.date.toDateString() === todayStart.toDateString());
            if (todayEntry) {
        ...
        logger_1.default.info(`Visitor count incremented to ${visitorData.totalVisitors} from IP: ${clientIP}`);
        res.status(200).json({
            success: true,
        ...
        logger_1.default.error("Error incrementing visitor count:", error);
        res.status(500).json({
            success: false,
 ...
 * @access  Public
 */
exports.getVisitorCount = (0, express_async_handler_1.default)(async (req, res) => {
    try {
        const visitorData = await visitor_1.default.findOne();
        if (!visitorData) {
        ...
        logger_1.default.error("Error getting visitor count:", error);
        res.status(500).json({
            success: false,
 ...
 */
exports.getVisitorStats = (0, express_async_handler_1.default)(async (req, res) => {
    try {
        const visitorData = await visitor_1.default.findOne();
        if (!visitorData) {
            res.status(200).json({
        ...
        const lastWeekVisits = visitorData.dailyVisits
            ...
            .reduce((sum, visit) => sum + visit.count, 0);
        const weeklyGrowth = lastWeekVisits > 0
        ...
        logger_1.default.error("Error getting visitor stats:", error);
        res.status(500).json({
            success: false,
 ...
 * @access  Private/Admin
 */
exports.resetVisitorCount = (0, express_async_handler_1.default)(async (req, res) => {
    try {
        const visitorData = await visitor_1.default.findOne();
        if (!visitorData) {
            res.status(404).json({
        ...
        visitorData.lastVisit = new Date();
        await visitorData.save();
        logger_1.default.info("Visitor count reset by admin");
        res.status(200).json({
            success: true,
        ...
        logger_1.default.error("Error resetting visitor count:", error);
        res.status(500).json({
            success: false,
